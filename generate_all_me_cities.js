...
// ==========================================
const CONFIG = {
    // 1. Target Countries (ISO 3166-1 alpha-2) - Middle East / MENA
    targetCountries: {
        'AE': 'UAE', 'SA': 'Saudi Arabia', 'QA': 'Qatar', 'OM': 'Oman', 
        'KW': 'Kuwait', 'BH': 'Bahrain', 'EG': 'Egypt', 'JO': 'Jordan', 
        'LB': 'Lebanon', 'TR': 'Turkey', 'IQ': 'Iraq', 'MA': 'Morocco'
    },
    ...
    middleEastDir: path.join(__dirname, 'Blogs', 'middle-east'),
    ...
    localDatasetPath: path.join(__dirname, 'me_cities.json')
};
...
function updateCountrySitemap(countrySlug, urls) {
    ...
    const sitemapPath = path.join(__dirname, `sitemap-middle-east-${countrySlug}.xml`);
...
async function main() {
    try {
        ...
        const baseUrl = 'https://ibrarzahoorr.github.io/Ibrar-Zahoor/Blogs/middle-east';

        ensureDirSync(CONFIG.middleEastDir);
        ...
        for (const countryCode of Object.keys(citiesByCountry)) {
            ...
            const countryDir = path.join(CONFIG.middleEastDir, countrySlug);
